...
export function resolveLinePrice(
  ...
  for (const group of requiredGroups) {
    const picked = selectedOptions.filter((selected) => selected.groupId === group.id);
    if (picked.length === 0) {
  ...
  for (const selected of selectedOptions) {
    const group = product.options.find((option) => option.id === selected.groupId);
    ...
    resolvedOptions.push({ groupId: group.id, choiceId: choice.id, name: choice.name });
  }
